 ...
 */
public class Embed implements Constants {
    ...
    public Embed(JPEGDecoder decoder, String password, String embedinfilename, String outimageFilename) {
    ...
    public int embedData(JPEGFrame frame, int comp222Number, int offsetForData, double BPNZ)
            throws Throwable {
        ...
        while (dataIndex < datareducedlimit) {
            ...
            for (int i = 0; i < posBooleanStop.length; i++) {
                for (int j = 0; j < posBooleanStop[0].length; j++) {
                    ...
                    if (posRunningCap[i][j] + codelength > posEstimatedCap[i][j] && !posBooleanStop[i][j]) {
                        posBooleanStop[i][j] = true;
                    }


                    if (negRunningCap[i][j] + codelength > negEstimatedCap[i][j] && !negBooleanStop[i][j]) {
                        negBooleanStop[i][j] = true;
                    }
                    if (negOnesRunningCap + codelength > negOneEstimated && !negOneStop) {
                        negOneStop = true;
                    }

                    if (posOnesRunningCap + codelength > posOneEstimated && !posOneStop) {
                        posOneStop = true;
                    }
